...
static int _FSS2_checkSizes(FSS2 *self){
	...
	if(marray3d_checkSizes(self->K,Nt,Nm,Np))	GMERR(-11);
	if(marray3d_checkSizes(self->Finv,Nt,Np,Np))GMERR(-21);
	if(marray3d_checkYZ(self->T,Nm,Nm))			GMERR(-31);
	if(marray3d_checkYZ(self->Q,Nr,Nr))			GMERR(-41);
	...
	if(matrixCheckSize(self->alpha,Nt,Nm))							GMERR(-81);
	...
	if(marray3d_checkX1orT(self->Z,Nt))								GMERR(-131);
	
	return(0);
GMERRH("_FSS2_checkSizes",1);
}
...
static int _FSS2_zeroOutputs(FSS2 *self){
	gsl_matrix_set_zero(self->alpha);
	gsl_matrix_set_zero(self->r);
	gsl_vector_set_zero(self->r_init);
	return(0);
GMERRH("_FSS2_zeroOutputs",1);
}
...
int FSS2_operations(FSS2 *self){
	...
	for(t=Nt-1;t>-1;t--){
		...
		if(Qt) A_ROW(Q,QQ,t);
		if(Zt) A_ROW(Z,ZZ,t);
		if(Zt)	if(gTranspose(ZZ,self->ZZT_Mmp))					GMERR(-12);
...
int FSS2_writeAlpha(FSS2 *self,char *name){
	...
	fp = fopen(name,"w");
	if(fp==NULL)				GMERR(-3);
	if(gsl_matrix_fprintf(fp,self->alpha,"%.15f"))					GMERR(-11);
	fclose(fp);
	return(0);
GMERRH("FSS2_writeAlpha",1);
}
