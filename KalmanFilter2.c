...
static int _Kalman2_checkSizes(Kalman2 *self);
static int _Kalman2_zeroOutputs(Kalman2 *self);

Kalman2 * Kalman2_New(){
	Kalman2 *self;
	self = (Kalman2 *)GM_Malloc(sizeof(Kalman2));
	if(self==NULL)								GMERR(-11);
	if(Kalman2_init(self))						GMERR(-21);
	return(self);
GMERRH("Kalman2_New",NULL);
}

int Kalman2_init(Kalman2 *self){
	if(self==NULL)								GMERR(-11);
	memset(self,'\0',sizeof(Kalman2));

	return(0);
GMERRH("Kalman2_init",1);
}
...
int Kalman2_Filter(gsl_matrix *y,gsl_vector *a_init,gsl_matrix *P_init,marray3d *Z,marray3d *H,marray3d *T,marray3d *R,marray3d *Q,gsl_matrix *v,marray3d *K,marray3d *Finv){
	Kalman2 *self = NULL;
	if((self=Kalman2_New())==NULL)				GMERR(-11);
	if(Kalman2_construct(self,y,a_init,P_init,Z,H,T,R,Q,v,K,Finv))	GMERR(-21);
	if(_Kalman2_checkSizes(self))				GMERR(-30);
	if(Kalman2_operations(self))				GMERR(-31);
	if(Kalman2_free(self))						GMERR(-41);

	return(0);
GMERRH("Kalman2_Filter",1);
}
...
static int _Kalman2_checkSizes(Kalman2 *self){
	...
	//Check inputs
	if(marray3d_checkYZ(self->Z,Np,Nm))						GMERR(-11);
	if(marray3d_checkYZ(self->H,Np,Np))						GMERR(-21);
	if(marray3d_checkYZ(self->T,Nm,Nm))						GMERR(-31);
	if(marray3d_checkYZ(self->R,Nm,Nr))						GMERR(-41);
	if(marray3d_checkYZ(self->Q,Nr,Nr))						GMERR(-51);
	if(matrixCheckSize(self->v,Nt,Np))						GMERR(-61);
	if(matrixCheckSize(self->P_init,Nm,Nm))					GMERR(-71);
	if(marray3d_checkX1orT(self->Z,Nt))						GMERR(-81);
	if(marray3d_checkX1orT(self->H,Nt))						GMERR(-91);
	if(marray3d_checkX1orT(self->T,Nt))						GMERR(-101);
	if(marray3d_checkX1orT(self->R,Nt))						GMERR(-111);
	if(marray3d_checkX1orT(self->Q,Nt))						GMERR(-121);
...
static int _Kalman2_zeroOutputs(Kalman2 *self){
	if(self==NULL)											GMERR(-1);
	gsl_matrix_set_zero(self->v);
	gsl_matrix_set_zero(self->a_Mtm);
	if(marray3d_set_zero(self->K))							GMERR(-11);
	if(marray3d_set_zero(self->Finv))						GMERR(-21);
	if(marray3d_set_zero(self->P_Atmm))						GMERR(-31);

	return(0);
GMERRH("_Kalman2_zeroOutputs",1);
}
...
int Kalman2_operations(Kalman2 *self){
	...
	gsl_matrix *HH,*TT,*RR,*QQ,*ZZ,*Pt_Mmm;

	//Zero the outputs
	if(_Kalman2_zeroOutputs(self))											GMERR(-11);
...
int Kalman2_writeFinv(Kalman2 *self,char *name){
	...
	fclose(fp);
	return(0);
GMERRH("Kalman2_writeFinv",1);
}

int Kalman_writeOutputs(Kalman2 *self,char *baseName){
	...
	if(self==NULL)						GMERR(-1);
	if(Kalman2_writeV(self,outV))		GMERR(-11);
	if(Kalman2_writeK(self,outK))		GMERR(-21);
	if(Kalman2_writeFinv(self,outFinv))	GMERR(-31);

	return(0);
GMERRH("Kalman2_writeOutputs",1);
}
