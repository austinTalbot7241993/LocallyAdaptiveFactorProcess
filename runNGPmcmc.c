...
int main(int argc,char **argv){
	const char *file1StringPtr;
	const char *file2StringPtr;
	int Nt = atoi(argv[3]);
	FILE *fp;
//	int nSamps = 10000;
	int i;
...
//	gsl_vector *epsSamps = gsl_vector_alloc(nSamps);
	...
	//Read inputs
	file1StringPtr = argv[1];
	file2StringPtr = argv[2];
	//Nt = atoi(argv[3]);
	printf("%d\n",Nt);
	...
	fp = fopen(file1StringPtr,"r");
	//fp = fopen("demeaned.txt","r");
	if(fp==NULL)			GMERR(-11);
	gsl_matrix_fscanf(fp,y);
	fclose(fp);
	printf("ahd;lfkjda;fl\n");

	fp = fopen(file2StringPtr,"r");
	//fp = fopen("TotalTimeHR2.txt","r");
	if(fp==NULL)			GMERR(-21);
	...
	if(NGPmcmc_writeOutputs(myMCMC,"Samps"))	GMERR(-101);
