...
marray3d_complex * marray3d_complex_alloc(size_t d1, size_t d2, size_t d3) {
    ...
    self->d2 = d2;
    self->d3 = d3;
    self->matrixList = (gsl_matrix_complex **)(GM_Malloc(d1 * sizeof(gsl_matrix_complex *)));
    if (self->matrixList == NULL) GMERR(-2);
    for (i = 0; i < (int)d1; i++) {
        self->matrixList[i] = gsl_matrix_complex_alloc(d2, d3);
        if (self->matrixList[i] == NULL) GMERR(-3);
    }
    return self;
GMERRH("marray3d_complex_alloc", NULL);
}

int marray3d_complex_free(marray3d_complex *self) {
    int i;
    if (self == NULL) return 0;
    if (self->matrixList != NULL) {
        for (i = 0; i < (int)self->d1; i++) {
            if (self->matrixList[i] != NULL) {
...
gsl_complex marray3d_complex_get(marray3d_complex *self, size_t x, size_t y, size_t z) {
    if (self == NULL || self->matrixList == NULL || x >= self->d1) {
        gsl_complex zero = gsl_complex_rect(0, 0);
        return zero;
    }
    return gsl_matrix_complex_get(self->matrixList[x], y, z);
}

int marray3d_complex_set(marray3d_complex *self, size_t x, size_t y, size_t z, gsl_complex val) {
    if (self == NULL || self->matrixList == NULL) GMERR(-1);
    if (x >= self->d1 || y >= self->d2 || z >= self->d3) GMERR(-11);
    gsl_matrix_complex_set(self->matrixList[x], y, z, val);
    return 0;
...
int marray3d_complex_set_identity(marray3d_complex *self) {
    ...
    return 0;
GMERRH("marray3d_complex_set_identity", 1);
}
...
int marray3d_complex_read(FILE *fp, marray3d_complex *self) {
    ...
    for (i = 0; i < d1; i++) {
        for (j = 0; j < d2; j++) {
            for (k = 0; k < d3; k++) {
                if (fscanf(fp, "%lf", &val_r) != 1) GMERR(-41);
                if (fscanf(fp, "%lf", &val_i) != 1) GMERR(-42);
                GSL_SET_REAL(&temp, val_r);
                GSL_SET_IMAG(&temp, val_i);
...
int marray3d_complex_set_Z(marray3d_complex *self, gsl_matrix_complex *m, int z) {
    ...
    return 0;
GMERRH("marray3d_complex_set_Z", 1);
}
...
int marray3d_complex_get_Y(marray3d_complex *self, gsl_matrix_complex *m, int y) {
    ...
    for (i = 0; i < d1; i++) {
        for (j = 0; j < d3; j++) {
            temp = gsl_matrix_complex_get(self->matrixList[i], y, j);
...
int marray3d_complex_get_Z(marray3d_complex *self, gsl_matrix_complex *m, int z) {
    ...
    for (i = 0; i < d1; i++) {
        for (j = 0; j < d2; j++) {
            temp = gsl_matrix_complex_get(self->matrixList[i], j, z);
            gsl_matrix_complex_set(m, i, j, temp);
        }
    }
    return 0;
