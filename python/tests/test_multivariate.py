...
def test_multivariate_lafp_model_basic():
    ...
    n_t = 30
    n_p = 3
    tobs = np.linspace(0.0, 5.0, n_t)
    
    # Generate 3-variate time series
    y = np.column_stack([
        np.sin(tobs) + 0.05 * np.random.randn(n_t),
        np.cos(tobs) + 0.05 * np.random.randn(n_t),
        np.sin(tobs) + np.cos(tobs) + 0.05 * np.random.randn(n_t),
    ])
    ...
    # 50 iterations thinned by 5 yields 10 saved samples
    n_samps = 10
...
def test_lafp_factor_model_auto_dispatch():
    ...
    np.random.seed(123)
    n_t = 25
    tobs = np.linspace(0.0, 4.0, n_t)
    y_2d = np.column_stack([
        np.sin(tobs),
        np.cos(tobs),
    ])

    model = LAFPFactorModel(n_iter=50, n_factors=2, n_dict=2)
    model.fit(y_2d, tobs)

    assert model.is_fitted_ is True
    assert model.mu_ is not None
    assert model.Sigma_ is not None
    assert model.mu_.shape == (10, n_t, 2)
    assert model.Sigma_.shape == (10, n_t, 2, 2)
    assert not np.isnan(model.mu_).any()
    assert not np.isnan(model.Sigma_).any()
